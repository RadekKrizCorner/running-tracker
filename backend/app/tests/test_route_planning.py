...
def test_valhalla_loop_payload_uses_conservative_waypoint_radius() -> None:
    """Verify loop waypoints are conservative enough for road-snapped routes."""
    from app.providers.routing.valhalla import build_valhalla_loop_payload
    from app.schemas.route_planning import RouteSuggestionRequest
    ...
    payload = build_valhalla_loop_payload(request, bearing_degrees=90)
    start = payload["locations"][0]
    waypoint = payload["locations"][1]
    waypoint_distance_m = _haversine_meters(start["lat"], start["lon"], waypoint["lat"], waypoint["lon"])

    assert 2400 <= waypoint_distance_m <= 2600
...
def test_route_suggestion_service_returns_local_demo_candidates() -> None:
    """Verify local demo routing returns approximate loop candidates."""
    from app.core.config import Settings
    from app.schemas.route_planning import RouteSuggestionRequest
    from app.services import route_planning_service

    settings = Settings(routing_enabled=True, routing_provider="local_demo")
    request = RouteSuggestionRequest(**_route_request_payload())

    response = route_planning_service.suggest_loop_routes(settings, uuid4(), request)

    assert response.status == "ok"
    assert response.detail == "Route suggestions generated by local demo routing."
    assert len(response.candidates) == 3
    assert response.candidates[0].provider == "local_demo"
    assert response.candidates[0].distance_m == pytest.approx(12000, abs=800)
    assert response.candidates[0].geometry[0] == pytest.approx((50.0755, 14.4378))
    assert response.candidates[0].geometry[-1] == pytest.approx((50.0755, 14.4378))
    assert "not snapped to roads" in response.candidates[0].warnings[0]


def test_local_demo_route_provider_reflects_hill_preferences() -> None:
    """Verify local demo routing reflects hill preference in route metadata."""
    from app.providers.routing.local_demo import generate_local_demo_loop_routes
    from app.schemas.route_planning import RouteSuggestionRequest

    flat_request = RouteSuggestionRequest(**{**_route_request_payload(), "hill_preference": "flat", "candidate_count": 1})
    hilly_request = RouteSuggestionRequest(**{**_route_request_payload(), "hill_preference": "hilly", "candidate_count": 1})

    flat_candidate = generate_local_demo_loop_routes(flat_request)[0]
    hilly_candidate = generate_local_demo_loop_routes(hilly_request)[0]

    assert flat_candidate.elevation_gain_m is not None
    assert hilly_candidate.elevation_gain_m is not None
    assert hilly_candidate.elevation_gain_m > flat_candidate.elevation_gain_m
...
def test_valhalla_request_keeps_searching_for_in_tolerance_candidates(monkeypatch: pytest.MonkeyPatch) -> None:
    """Verify Valhalla search continues after out-of-tolerance candidates."""
    from app.providers.routing import valhalla
    from app.schemas.route_planning import RouteSuggestionRequest
    ...
    client = DistanceSequenceClient([13.2, 10.4])
    monkeypatch.setattr(valhalla.httpx, "Client", lambda timeout: client)

    candidates = valhalla.request_valhalla_loop_routes("http://valhalla.test", request)

    assert client.calls == 2
    assert candidates[0].distance_m == pytest.approx(10400)
    assert candidates[0].id == "valhalla-1"
...
class DistanceSequenceResponse:
    """Represent a fake Valhalla response with one distance."""

    def __init__(self, distance_km: float) -> None:
        """Store the fake route distance."""
    ...
    def json(self) -> dict[str, object]:
        """Return a fake Valhalla route payload."""
        return {
            "trip": {
                "status": 0,
                "summary": {"length": self.distance_km, "time": int(self.distance_km * 360)},
                "legs": [{"shape": "wujo~AoyepZw|A_gE~iA_gE", "summary": {"length": self.distance_km}}],
            }
        }


class DistanceSequenceClient:
    """Represent a fake Valhalla HTTP client returning distances in order."""

    def __init__(self, distances_km: list[float]) -> None:
        """Store fake route distances."""
        self.distances_km = distances_km
        self.calls = 0

    def __enter__(self) -> "DistanceSequenceClient":
        """Enter the fake HTTP client context."""
        return self
    ...
    def post(self, _url: str, json: dict[str, object]) -> DistanceSequenceResponse:
        """Return the next fake Valhalla response."""
        _ = json
        distance_km = self.distances_km[min(self.calls, len(self.distances_km) - 1)]
        self.calls += 1
        return DistanceSequenceResponse(distance_km)


def _haversine_meters(start_lat: float, start_lng: float, end_lat: float, end_lng: float) -> float:
    """Return the approximate distance between two coordinates."""
