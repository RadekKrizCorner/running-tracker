...
def suggest_loop_routes(
    ...
    if not settings.routing_enabled:
        return _unavailable("Route suggestions are disabled. Enable local demo routing or configure local Valhalla.")
    ...
    if settings.routing_provider == "local_demo":
        return _suggest_local_demo_routes(request)
...
def _suggest_local_demo_routes(request: RouteSuggestionRequest) -> RouteSuggestionResponse:
    """Return local demo route suggestions."""
    capped_request = request.model_copy(update={"candidate_count": min(request.candidate_count, 6)})
    candidates = _usable_candidates(generate_local_demo_loop_routes(capped_request), request)
    if not candidates:
        return _unavailable(UNUSABLE_CANDIDATES_DETAIL)
    return RouteSuggestionResponse(
        status="ok",
        detail="Route suggestions generated by local demo routing.",
        candidates=_limit_candidates(candidates, capped_request.candidate_count),
    )
